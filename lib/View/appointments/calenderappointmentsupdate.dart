...
class StylishCalendarScreenUpdate extends StatefulWidget {
  final Appointment appointment; // Add this line to accept property details
  const StylishCalendarScreenUpdate({Key? key, required this.appointment})
      : super(key: key);

  @override
  _StylishCalendarScreenUpdateState createState() =>
      _StylishCalendarScreenUpdateState();
}

class _StylishCalendarScreenUpdateState
    extends State<StylishCalendarScreenUpdate> {
    ...
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
      content: Text(
          "Réservation confirmée pour le ${DateFormat('dd-MM-yyyy').format(_selectedDay!)}." +
              " Merci de consulter l'espace de vos rendez-vous pour voir le statut de votre rendez-vous."),
...
// This should come from your user session or similar
    ...
    final appointmentData = {
      ...
      'idproperty': widget.appointment.propertyId,
      'dateAppointment': _selectedDay!.toIso8601String(),
      'status': 'En attente'
    };

    BlocProvider.of<AppointmentBloc>(context).add(UpdateAppointmentEvent(
    ...
    // Close the bottom sheet modal after the dialog
